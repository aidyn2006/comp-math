...
def compute_g(equation):
    g = sp.solve(sp.Eq(sp.sympify(equation), 0), x)
    if len(g) > 0:
        return g[0]
    else:
        raise ValueError("Unable to compute g(x). Please check the equation.")

g_expr = compute_g(equation)

def g(x_val):
    return float(g_expr.subs(x, x_val))
...
def iteration_method(g, x0, tolerance, max_iter=100):
    x_values = [x0]
    count = 0
...
x1, x2 = find_interval(f, start=-10, end=10, step=0.1)
...
newton_raphson_x, newton_raphson_y = newton_raphson_method(f, f_prime, (x1 + x2) / 2, tolerance)
print("Newton-Raphson Method: \n approximately root:", newton_raphson_x[-1], "number of iterations:", newton_raphson_y[-1])

iteration_x, iteration_y = iteration_method(g,(x1 + x2) / 2, tolerance)
